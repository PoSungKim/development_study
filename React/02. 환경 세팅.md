...
touch tsconfig.json
```
* ts-loader는 필요없음. 추후에 babel-loader Plug-in을 설치해서 Transpiling은 Babel이 수행할 예정
...
```sh
yarn add -D webpack webpack-cli @types/webpack webpack-dev-server @types/webpack-dev-server babel-loader ts-node html-webpack-plugin style-loader css-loader normalize.css mini-css-extract-plugin file-loader sass sass-loader
...
module.exports = {
  ...
  devServer: {
    contentBase: path.resolve(__dirname, "build"), // where to set the root directory
    compress: true,
...
#### 7. build용, start용 npm scripts 넣기 (package.json)
