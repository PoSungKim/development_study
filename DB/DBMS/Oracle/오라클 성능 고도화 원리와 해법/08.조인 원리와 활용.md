...
* 10g에서 버퍼 Pinning 효과
  * Inner 쪽 테이블을 Index Rnage Scan을 거쳐 NL 조인 위쪽엥서 액세스할 때는, 하나의 Outer 레코드에 대한 Inner 쪽과의 조인을 마치고 Outer 쪽으로 돌아오더라도 테이블 블록에 대한 Pinning 상태를 유지한다
  ```sql
  ...
  1      NESTED LOOPS (cr=1409213 pr=0 pw=0 time=...)
  1278     TABLE ACCESS FULL T_EMP (cr=9211 pr=0 pw=0 time=...)
  5        TABLE ACCESS BY INDEX ROWID DEPT (cr=1400002 pr=0 pw=0 time=...)
  8          INDEX UNIQUE SCAN DEPT_PK (cr=2 pr=0 pw=0 time=...) 
  ```
* 11g에서 버퍼 Pinning 효과
  * ㅇㅇ 
