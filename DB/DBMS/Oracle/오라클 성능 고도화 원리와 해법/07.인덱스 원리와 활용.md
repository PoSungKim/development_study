...
* 인덱스 클러스터는 넓은 범위를 검색할 때 유리
  * 일반 B*Tree 인덱스 + 힙 구조 테이블
    ...
    ```
    * 인덱스에서 1763 Random Access > 테이블에서 636 블록 I/O
    * SORT AGGREGATE
...
* 선분이력 스캔 효율을 높이는 방법 요약
  * 선분이력처럼 between 검색 조건이 사용될 때, 어느 시점을 주로 검색하는가에 따라서 다른 전략을 가져가야 한다
    * 최근 데이터 : 인덱스(종료일, 시작일) w/ index_desc && rownum <= 1
    * 오래된 과거 데이터 : 인덱스(시작일, 종료일)
  * rownum과 index_desc 힌트를 적절히 사용하면 인덱스 구성이 어떻든지 간에 항상 필요한 건만 스캔하도록 할 수 있다

<br>

### Access Predicate와 Filter Predicate
* 인덱스를 경유해 테이블을 액세스할 때 3가지의 Predicate 정보가 나타날 수 있다
  * 인덱스 단계에서의 Access Predicate
  * 인덱스 단계에서의 Filter Predicate
  * 테이블 단계에서의 Filter Predicate 
* 인덱스를 경유하지 않고 테이블 전체를 스캔할 때 1가지의 Predicate 정보가 나타날 수 있다
  * 테이블 단계에서의 Filter Predicate
* 인덱스 단계에서의 Access Predicate에 미포함되는 조건
  * 좌변 칼럼을 가공한 조건절
  ```sql
  trim(e.ename) = 'ALLEN'
  ```
  * 왼쪽 % 또는 양쪽 % 기호를 사용한 like 조건절
  ...
  ename like '%A%'
  ```
  * 같은 칼럼에 대한 조건절이 두 개 이상일 때, 인덱스 액세스 조건으로 선택되지 못한 다른 조건걸
  ```sql
  job between 'A' and 'Z'
  ```
* 인덱스 단계에서의 Access Predicate
  * 수직적 탐색 과정에서 모든 인덱스 칼럼을 비교 조건으로 사용한다
  * 인덱스 스캔 범위를 결정
* 인덱스 단계에서의 Filter Predicate
  * 테이블로의 인덱스 여부를 결정짓는 조건절을 의미
  * 범위검색 조건부터 이후 모든 조건절 칼럼들이 여기에 포함
  * 조건절에서 누락된 칼럼 뒤쪽에 놓인 인덱스 칼럼들도 포함
* 테이블 단계에서의 Filter Predicate
  * 테이블을 액세스하고 나서 최종 결과집합으로의 포함여부를 결정짓는 조건절을 의미 

<br>

### Index Fragmentation
* B*Tree 인덱스는 Balanced 상태를 유지하기 때문에 항상 루트 노드부터 모든 리프 블록까지의 높이(height)가 동일
* Index Skew
  * 인덱스 엔트리가 왼쪽 또는 오른쪽에 치우치는 현상
  ...
  commit; -- 삭제된 블록들이 freelist로 반환되는 시점
          -- 인덱스 왼쪽에 있는 텅빈 리프 블록들은 freelist로 반환
          -- 인덱스 오른쪽에 있는 리프 블록들은 꽉 찬 상태가 된다
  ```
  * 텅 빈 인덱스 블록은 커밋하는 순간 freelist로 반환되지만 인덱스 구조 상에는 그대로 남는다
  * 레코드가 모두 삭제된 블록은 언제든 재사용이 가능하지만, 남아 있기 때문에 인덱스 스캔 효율이 낮다
  * delete 후 바로 commit해서 freelist로 반환하고, freelist에 있는 블록들에 다시 insert하는 예시)
    * delete 후 commit을 해야지 freelist가 되기 때문에, commit 시점을 앞당긴 사례
  ```sql
...
* Index Sparse
  ...
  * 아예 빈 블록이 아니라, 덜 채워진 블록들이기 때문에, 새로운 값이 입력되지 않는 이상 freelist로 반환되어 재사용되지 않는다
  * 총 레코드 건수가 일정한데도 인덱스 공간 사용량이 계속 커지는 것는 대개 이런 현상에 기인한다
* Index Rebuild
  * Fragmentation 때문에 인덱스 크기가 계속 증가하고 스캔 효율이 나쁠 때는 아래와 같이 coalesce 명령을 수행해 주면 된다
  * 여러 인덱스 블록을 하나로 병합(merge)하고, 생긴 빈 블록들을 freelist에 반환한다
  ```sql
  ...
  ```
  * 인덱스 블록의 공간을 너무 안 남겨놓으면 인덱스 분할이 자주 발생해 DML 성능을 떨어뜨린다
    * `Shared 모드 enq : TX - index contention` 대기 이벤트가 자주 발생
  * Rebuild 연산도 시스템에 주는 부하가 적지 않기 않고, online rebuild 기능을 이용하더라도 트랜잭션에 다소 경합을 일으킨다
  * 아래와 같이 확실한 예상효과가 확실할 때는 인덱스 Rebuild 고려 필요
    * 인덱스 분할에 의한 경합이 현저히 높을 때
    * 자주 사용되는 인덱스 스캔 효율을 높이고자 할 때. 특히 NL 조인에서 반복 액세스되는 인데스 높이(height)가 증가했을 때
    * 대량의 delete 작업을 수행한 이후 다시 레코드가 입력되기까지 오랜 기간이 쇼요될 때
    * 총 레코드 수가 일정한대도 인덱스가 계속 커질 때
  * 인덱스 freelist
    * PCTFREE
      * freelist에서 제외되는 시점을 지정하는 파라미터
      * 테이블 : 블록에 더 이상 insert가 발생하지 못하도록 freelist로부터 제외되는 시점을 지정한다 > 나중에 update를 위해 사용한다
      * 인덱스 : 인덱스가 생성되는 시점에 공간을 꽉 채워두면 나중에 인덱스 분할이 빈번하게 발생하므로 이를 방지하려고 pctfree가 필요하다 > 인덱스 최초 생성 또는 재생성 시점에만 적용되며, 지정한 비율만큼 공간을 남겨두었다가 나중에 insert를 위해 사용한다
    * PCTUSED
      * freelist에서 다시 등록되는 시점을 지정하는 파라미터
      * 테이블 : freelist에서 제거된 테이블 블록에 빈 공간이 일정 수준 이상 확보됐을 때만 다시 freelist에 등록되도록 하기 위해 pctused 파라미터가 필요하다
      * 인덱스 : pctused 파라미터가 없다 > 인덱스에서 빈 공간은 항상 재사용 가능하기 때문
...
## 인덱스 설계
#### 인덱스 스캔방식, Random 액세스 최소화 원리, 스캔 효율화 원리를 기초로 효과적인 인덱스 설계 방안 필요

<br>

### 가장 중요한 두 가지 선택 기준
* 조건에 항상 사용되거나, 자주 등장하는 칼럼들을 선정한다
  * 가장 정상적이고 일반적인 것은 Index Range Scan > 인덱스 선두 칼럼이 조건절에 반드시 사용되어야 한다
* = 조건으로 자주 조회되는 칼럼을 앞쪽에 두어야 한다
  * 범위검색이 앞쪽에 위치하면 데이터가 분산된다

<br>

### 인덱스 설계는 공식이 아닌 전략과 선택의 문제
* 통용될 수 있는 기본 공식이긴 하나, 지키려고 하다보면, 오히려 기준이 없을 때처럼 인덱스 개수가 자꾸 늘어나는 현상을 겪을 수 있다
* 간단한 설계 예제
  ```sql
  -- 검색조건 1
  where 고객번호 = 1
  ...
  ```
* 인덱스 설계 스타일별 평가
  * A) = 조건 칼럼을 모두 선두에 둠으로써 모든 조건에 대해 인덱스 스캔 효율을 100% 달성하려는 스타일
    * DML 부하 및 관리비용 증가
    * 이미 너무 많아진 인덱스
  * B) 고객번호별 조회 및 상품번호별 조회를 둘 다 중요 액세스 경로 판단
    * 검색조건 3일 때, X1의 고객번호의 번별력이 좋아서 테이블 액세스를 최소화할 수 있다고 판단한 것으로 보임
  * C) B와 같은 스타일
  * D) B와 같은 스타일
    * 다만, 상품번호의 선택도가 고객번호보다 높아서 스타일 C와 비교해 인덱스 스캔량이 더 많을 것
  * E) between 조건을 최선두에 둔다면 나머지 조건은 거의 인덱스 필터 역할만 하므로 검색조건 1~4만 있는 환경에서는 X1과 X2 중 하나는 삭제해도 됨
  * F) 검색조건 1~4에 대해서 효과적이지 못한 스타일
  * G) 상품번호로 조회하는 검색조건2과 고객번호로 조회하는 검색조건 1에서도 인덱스 스캔 비효율이 생긴다
  * H) 검색조건 2,4에 대한 대비가 없고, 검색조건 1에서도 인덱스 스캔 비효율이 크다
  * I) X2 인덱스 선두에 거래일자를 둠으로써 약간의 비효율은 생기겠지만 항상 사용되는 칼럼이므로 모든 검색조건에 범용적으로 사용될 수 있다 + 고객번호가 조회 조건에 포함될 때는 X1 인덱스를 효과적으로 이용하려는 의도가 엿보인다
    * 모든 검색 조건을 만족하면서도 인덱스 개수를 최소화했다는 측면에서 고평가 가능
  * J) 인덱스 하나로 모든 조건절을 해결하려는 의도가 담긴 것 같다
    * 근본적인 비효율을 안고 있어 신중히 선택해야 할 설계 방식

| 스타일 | X1                             | X2                             | X3                             | X4       |
|--------|--------------------------------|--------------------------------|--------------------------------|----------|
| A      | 고객번호 + 거래일자            | 상품번호 + 거래일자            | 상품번호 + 고객번호 + 거래일자 | 거래일자 |
| B      | 고객번호 + 거래일자            | 상품번호 + 거래일자            | 거래일자                       |          |
| C      | 고객번호 + 거래일자 + 상품번호 | 상품번호 + 거래일자            | 거래일자                       |          |
| D      | 고객번호 + 거래일자            | 상품번호 + 거래일자 + 고객번호 |                                |          |
| E      | 거래일자 + 상품번호 + 고객번호 | 거래일자 + 고객번호 + 상품번호 |                                |          |
| F      | 거래일자 + 고객번호            | 거래일자 + 상품번호            |                                |          |
| G      | 거래일자 + 상품번호            | 고객번호 + 상품번호 + 거래일자 |                                |          |
| H      | 고객번호 + 상품번호 + 거래일자 |                                |                                |          |
| I      | 고객번호 + 거래일자            | 거래일자 + 상품번호 + 고객번호 |                                |          |
| J      | 거래일자 + 상품번호 + 고객번호 |                                |                                |          |

* 효율성 비교 분석
  * 테이블에 insert된 데이터는 정렬되어 있지 않다
  * 인덱스에 미포함된 칼럼을 select하기 위해서는 Table Scan이 필요하고 이때 Random Access가 발생한다
  * 클러스터링 팩터를 고려하지 않는 상태에서는 이론적으로 Random Access 1회 발생 시, 테이블 블록 1번을 읽었다고 평가할 수 있다
  * 테이블 블록 스캔 개수 + 인덱스 리프 블록 스캔 개수 모두 고려하여 효율성을 평가해야 한다
  * 인덱스 스캔 범위를 줄이면서 인덱스 리프 블록 스캔 개수를 감소시킬 수 있는데, 결과적으로 테이블 블록 스캔 개수는 동일할 수도 있고, 효율성에 큰 변화가 없는 결과로 이어질 수도 있다
* 스캔 효율성 이외의 판단 기준
  * NL조인의 Inner Table로서 액세스 횟수
  ```sql
  select /*+ ordered user_nl(b) */
        a.상품명, a.고객번호, a.거래일자, b.거래량, b.거래금액
  from    상품 a, 거래 b 
  where   a.상품분류 = '가전'
  and     b.상품번호 = a.상품번호
  ...
  * 인덱스 관리 비용 등
* 인덱스 설계는 공식이 아닌 전략과 선택의 문제
  * 10개의 조건절 조합에 맞춰서 매번 10개의 인덱스를 생성할 수는 없다
  * 핵심적인 액세스 경로 (Critical Access Path)한두 개를 전략적으로 선택하고, Range 파티션 등 파티션 설계를 먼저하여 최소한의 인덱스 설계와 병행하는 것 바람직
* 인덱스 전략 수립을 위한 훈련
  * 고객 100만 + 거주지역 15개 (균일 분포) + 쿼리1 및 쿼리4 수행빈도 매우 높음 + 쿼리2 및 쿼리3 수행빈도 매우 낮음
  ```sql
  -- 쿼리 1
  select * from 고객
  where  고객번호 = :no;
  ...
  * 인덱스 설계 조정을 통한 튜닝 사례 - Right Growing 인덱스
    * 인덱스에서 키 값 기준으로 정렬된 상태를 유지하며 값이 입력(insert)된다
    * `일련번호`나 `입력일시`처럼 순차적으로 증가하는 칼럼에 생성한 인덱스는 항상 맨 우측 블록으로만 값이 입력된다
    * 이런 특징을 갖는 인덱스가 Right-Growing 인덱스이며, 여러 프로세스에 의한 동시 입력이 많을 때 트랜잭션 성능을 떨어뜨리는 주범이다
      * 입력값이 다르므로 로우 Lock 경합은 발생하지 않는다
      * 같은 블록을 갱신하려는 프로세스 간 버퍼 Lock 경합은 발생한다 (buffer busy waits 이벤트 발생)
      * 여러 노드가 동시에 Current 블록 하나를 서로 주고 받으며 값을 insert 해야 하기 때문에 RAC 환경에서 gc buffer busy 이벤트로 관찰된다
  * 해결방법
    * Reverse Key 인덱스로 바꿔 블록을 분산하는 방법
    * 인덱스를 해시 파티셔닝하는 방법
      * 값이 순차적으로 증가하더라도 해시 함수에서 리턴된 값에 따라 서로 다른 파티션에 입력되므로 경합을 그만큼 줄일 수 있다
    * 예시 상황)
      * 인덱스
      ...
      * N1 : (변경일자, 서비스변경코드)에서 Hot Block 경합이 발생하는 이슈 발생
        * N1 인덱스에 번별력이 좋은 고객번호 칼럼을 추가하여, rowid 이전에 고객번호에 따라 여러 블록에 골고루 분산되는 효과를 기대했음
        * (변경일자, 서비스변경코드) 별로 데이터가 엄청 많을 텐데, 인덱스를 통해 액세스했을 때 제대로 속도가 나올지에 대한 의구심이 생김 
        * 따라서, 단 하나의 쿼리에서 N1 인덱스를 사용하고 있음을 확인했고, 판매부서id 조건에 강제로 추가하여 N2 인덱스를 타도록 수정하여 이슈 해결

<br>

### 결합 인덱스 칼럼 순서 결정 시, 선택도 이슈
* 항상 선택도가 낮은, 즉 변별력이 높은 족을 앞에 두는 것이 유리하지는 않다
* 선택도가 액세스 효율에 영향을 주지 않는 경우
  * = 조건으로 항상 사용되는 칼럼들을 앞쪽에 위치시켰다면, 그 중 선택도가 낮은 것을 앞쪽에 두려는 노력은 의미 없는 것이거나 오히려 손해일 수 있다
  * 수직적 탐색 과정에서는 모든 인덱스 칼럼을 비교 조건으로 사용하므로 스캔 시작 지점은 어느 것을 앞에 두더라도 동일하게 결정된다
  * 예시)
    * IDX01 : 고객등급 + 고객번호 + 거래일자 + 거래유형 + 상품번호
    * 고객등급(=), 고객번호(=), 거래일자(between)
* 선택도가 높은 칼럼을 앞쪽에 두는 것이 유리한 경우
  * 선택도가 높은 고객등급을 선두 칼럼으로 두면
    * 나중에 조건절에서 누락되거나 범위검색 조건으로 조회되는 일이 생기더라도 Index Skip Scan이나 IN-LIST로 값을 제공함으로써 튜닝할 여지가 생긴다
    * 인덱스 압축률도 더 좋아진다
* 상황에 따라 유불리가 바뀌는 경우
  * 종합해보면, 선택도가 높은 칼럼(고객등급)을 선두에 두면 나중에 범위검색 조건이 사용되거나 아예 조건절에서 누락되더라도 Index Skip Scan 또는 IN-List를 활용할 수 잇어 유리하다. 다만, 이들 기법이 효과를 발휘할 정도로 Distinct Value 개수가 추분히 적은지가 관건이다
  * 선택도가 낮은 칼럼(고객번호)을 선두에 두면 이를 범위검색 조건으로 조회하는 일이 생겼을 때 불리하다
    * 다만, 입력 값의 범위가 좁다면 비효율이 크지 않아 Index Skip Scan이나 IN-List를 활용하지 못하더라도 오히려 유리할 수 있다

| 고객번호 | 고객등급 | (고객번호, 고객등급, 거래일자)                                                                                                                                                    | (고객등급, 고객번호, 거래일자)                                                                               |
|----------|----------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------|
| >=       | =        | 스캔 범위가 넓어져 불리하지만, 고객번호에 입력되는 값의 범위가 좁을 때는 비효율이 크지 않음.<br>입력 범위가 넓을 때가 문제인데, ISS 또는 IN-List 기법을 활용하지 못하는 단점 있음 | 유리                                                                                                         |
| =        | >=       | 유리                                                                                                                                                                              | 선택도가 높은 고객등급에 의해 스캔범위가 결정되므로 불리하지만, ISS 또는 IN-LIST를 활용한다면 큰 비효율 없음 |
| >=       | >=       | 고객번호에 입력되는 값의 범위가 좁을 때는 비효율이 크지 않음.<br>입력 범위가 넓을 때가 문제인데, ISS 또는 IN-List 기법을 활용하지 못하는 단점 있음                                | 선택도가 높은 고객등급에 의해 스캔범위가 결정되므로 불리하지만, ISS 또는 IN-List를 활용한다면 큰 비효율 없음 |
| =        | (누락)   | 유리                                                                                                                                                                              | ISS 또는 IN-List 활용 가능                                                                                   |
| (누락)   | =        | Full Table Scan이 유리                                                                                                                                                            | Full Table Scan이 유리                                                                                       |

* 선택도가 낮은 칼럼을 앞쪽에 두는 것이 유리한 경우
  * 범위검색 조건을 사이에 둔 칼럼끼리는 선택도가 낮은 칼럼을 앞쪽에 두는 것이 유리
  ```sql
  where 고객번호 = :a and 상품번호 = :b and 거래일자 between :c and :d
  ```
  * 고객 (1,000,000), 상품개수 (10,000)
    * 최하단 칼럼은 인덱스 스캔 필터링 용도로만 사용되서, 선두 칼럼에서라도 선택도가 낮아야지 스캔 범위가 최소화 된다
    * 보통 이런 칼럼 구성을 가진 인덱스가 발생한 이유로는 (고객번호 = 조건, 거래일자 between 조건)
  ```sql
  X01 : (고객번호, 거래일자, 상품번호) -- 상품번호가 인덱스 스캔 단계에서 필터 역할만 한다
  X02 : (상품번호, 거래일자, 고객번호) -- 고객번호가 인덱스 스캔 단계에서 필터 역할만 한다
  ```
* 선택도가 낮은 칼럼을 선택하는 것이 유리한 경우
  * 아래와 같은 조건절 하에서 만약 거래일자를 선두에 둔 '단 하나'의 인덱스를 생성하기로 하셨고, 후행 칼럼으로서 상품번호와 고객번호 둘 중 하나만 '선택'해야 한다면, 선택도가 낮은 고객번호를 선택해야 한다
  ```sql
  -- 검색조건1
  ...
  ```
  * 물론, 더 많이 사용되는 칼럼이 1순위이어야 하지만, 사용빈도가 동일하면 선택도가 낮은 칼럼을 선택해야 한다
  * 선택도가 낮은 고객번호로 인덱스를 사용해야 더 적은 양의 테이블 Random 액세스가 발생하기 때문
  * 이렇게 특별한 케이스에서나 선택도를 비교해서 만들지, 실제 상황에서 결합 인덱스 칼럼 간 순서를 정할 때는 개별 칼럼의 선택도보다는 조건절에서 어떤 형태로 자주 사용되는지, 사용빈도는 어느 쪽이 높은지, 데이터를 빠르게 검색하는 데에 어느 쪽 효용성이 높은지 등이 더 중요한 판단기준이다

<br>

### 소트 오퍼레이션을 생략하기 위한 칼럼 추가
* 인덱스는 항상 정렬 상태를 유지하므로 order by, group by를 위한 소트 연산을 생략할 수 있게 해준다
  * 소트 연산을 대체하기 위해서는 인덱스 칼럼 구성과 같은 순서로 누락 없이(뒤쪽 칼럼이 누락되는 것은 상관없음) order by절에 기술해 주어야 한다
* 인덱스 구성 칼럼이 조건절에서 = 연산자로 비교된다면, 그 칼럼은 order by/group by절에서 누락되거나 인덱스와 다른 순서로 기술하더라도 상관없다
```sql
...
order by a, b, c;
```
* 정렬 작업이 생략되지만, 인덱스 선두 칼럼이 조건절에 누락됐으므로 Index Full Scan으로 처리된다
```sql
...
* 인덱스 설계도 작성을 통한 튜닝 사례
  * 인덱스 선두 칼럼에 입력일 범위 검색 조건 같은걸로 짜지 말자
...
#### 비트가 1로 설정돼 있으면 상응하는 테이블 레코드가 해당 키 값으 포함하고 있음을 의미한다
