...
* 파티션 Pruning 기능에 따른 I/O 수행량 비교
  * 주 파티션과 서브 파티션에 Pruning이 작동해 하나의 서브 파티션에서 49개 블록만 읽은 예시)
    ```sql
    ...
    ```
  * 서브 파티션 키 칼럼을 함수로 가공하여 서브파티션에 Pruning이 작동하지 못해서 16개 서브 파티션에서 776개 블록을 읽은 예시)
    * 즉, 파티션 키 칼럼도 함부로 가공하면 안 된다
    ...
    ------   ----------------------------------------------------------------------------
         ...
         1     PARTITION HASH SINGLE PARTITION: 1 16 (cr=776 pr=528 pw=0 time=1056027 us)
         1       TABLE ACCESS FULL T PARTITION: 1 16 (cr=776 pr=528 pw=0 time=1055868 us)
    ```
  * 묵시적 형변환으로 인해 파티션 키 칼럼이 명싱적으로 가공했을 때와 동일한 블록 I/O가 발생한 예시)
    ```sql
    ...
    ------   ------------------------------------------------------------------------------
         ...
         1     PARTITION HASH SINGLE PARTITION: 1 16 (cr=776 pr=528 pw=0 time=954945 us)
