...
* OLTP성 애플리케이션 환경
  ...
  * 파티션 키 칼럼만으로 조회할 때는 로컬 Prefixed 파티션 인덱스를 만들면 된다
    ...
    ```
  * 정리
    * 로컬 파티션 인덱스를 사용한다면, 검색 조건에 항상 사용되는 칼럼을 파티션 키로 선정해야 한다
      * 파티션 키가 범위검색 조건으로 자주 사용된다면 로컬 Nonprefixed 파티션 인덱스가 유리
    * 만약, NL 조인에서 파티션 키에 대한 넓은 넘위검색 조건을 가지고 Inner 테이블 액세스 용도로 사용되면 비파티션 인덱스가 유리
  
<br>

### (10) 인덱스 파티셔닝 제약을 고려한 데이터베이스 설계
* 인덱스 파티셔닝 핵심 제약조건
  * Unique 파티션 인덱스를 정의할 때는 인덱스 파티션 키가 모두 인덱스 구성 칼럼에 포함되어야 한다
  * 글로벌 파티션 인덱스는 Prefixed 파티션이어야 한다
* 테이블 파티셔닝을 고려하고 있다면 파티션 기준이 되는 구분자 칼럼을 물리 설계 단계에서 PK 칼럼에 포함시키는 것이 좋다
  * 최종적인 물리 설계 단계에서도 상품대분류 (파티션 기준 구분자 칼럼)를 그대로 일반 속성으로 두었을 때 부딪히는 제약사항

    <div align="center">
      <img width="50%" src="https://github.com/PoSungKim/development_study/assets/37537227/ab3c6fe4-cb2b-426e-b87d-c26865fe746b" >
    </div>
    ...
    -- 에러발생 : ORA-14188 : sub-partitioning columns must form a subset of key columns of a UNIQUE index
    -- 파티션 키인 상품대분류가 인덱스 칼럼에 미포함되면서 에러 발생
    ```
    ...
    partition by RANGE( 거래일자 ) ( ... );
    -- 에러발생 : ORA-14038 : GLOBAL partitioned index must be prefixed
    -- (거래일자, 상품번호) 순으로 인덱스 정렬 순서를 정의하라는 의미이지만, 범위검색 조건을 선두 칼럼으로 두는 것은 곤란하다
    -- 아예 PK 인덱스를 파티셔닝하지 않고 비파티션 인덱스로 생성해도 된다
    -- 아니면, PK 인덱스로 Prefixed 인덱스를 사용하되, 추가로 로컬 Nonprefixed 인덱스를 하나 더 만들어도 된다면 그렇게 진행해도 된다
    -- 물론, Trade-off (between 성능과 관리비용)를 고려하여, 하나의 인덱스를 사용하고자 한다면, 제약사항으로 인해 2개를 사용하게 되는 것 또한 곤란하다
    ```
    
  * 제약사항을 피해가기 위해 PK에 상품대분류 (파티션 기준 구분자 칼럼)를 포함시킴으로써 제약사항을 피할 수 있다

    <div align="center">
      <img width="50%" src="https://github.com/PoSungKim/development_study/assets/37537227/5731a7d6-d271-4614-8bc5-3448373cb80b" >
    </div>
    
    * 상단의 PK 인덱스 문제를 로컬 Nonprefixed 방식으로 파티셔닝할 수 있게 된다
    ```sql
    create unique index 일별상품거래_PK
    ...
    * PK 제약을 위해서 사용 가능
    ...
    ```
    * (상품번호 =, 거래일자 between) 조건으로 조회할 때도 사용 가능
    ```sql
    select *
    from   일별상품별거래
    where  상품번호 = :isu_cd
    ...
    ```
    * (상품대분류, 거래일자)로만 조회할 때는 한 개 또는 일부 서브파티션만 Full Scan하면 되기 때문에 비효율 없이 다량의 데이터를 조회할 수 있다
    ```sql
    select *
    from   일별상품별거래
