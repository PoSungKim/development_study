...
* INDEX RANGE SCAN > A-Rows
  * 인덱스 Access 조건 + 인덱스 Filtering 조건 후 남은 Rows 수 >> 테이블 랜덤 액세스 대상이 되는 Rows 수
* TABLE ACCESS BY INDEX ROWID > A-Rows
  * 테이블 Access 조건 + 테이블 Filtering 조건 후 남은 Rows 수 >> 아우터 테이블 혹은 아우터 테이블의 인덱스 액세스 대상이 되는 랜덤 액세스 대상이 되는 Rows 수
