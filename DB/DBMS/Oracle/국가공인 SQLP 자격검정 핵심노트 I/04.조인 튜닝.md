...
### 스칼라 서브쿼리
* select문 내부 스칼라 서브쿼리에서도 NL조인이가능하다
* 문제
...
* 대용량 데이터를 조인할 때 NL조인보다 유리
  ...
  * 차이점
    * NL 조인
      * 인덱스를 이용한 조인 방식
      * 조인 과정에서 액세스하는 모든 블록을 랜덤 액세스 방식으로 `건건이` DB버퍼캐시를 경유해서 읽는다
    * 소트 머지 조인
      * 양쪽 테이블로부터 조인 대상 집합을 `잏괄로` 읽어 PGA(또는 Temp 테이블스페이스)에 저장한 후 조인한다
