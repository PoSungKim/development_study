...
* Join Predicate Pushdown 케이스
  * 따라서, group by전 고객번호 조인 연산을 선행함으로써 부분범위 처리가 되도록 튜닝 가능
  ```sql
...
* 원본 쿼리
  ...
  1        HASH (GROUP BY) 
  2          HASH JOIN (OUTER)
  3            TABLE ACCESS (BY INDEX ROWID) OF '고객'
  3            TABLE ACCESS (BY INDEX ROWID) OF '거래'
  ```
* Unnesting 케이스
