...
* NL조인 && Hash조인 차이점
  ...
  * Hash조인은 PGA에 미리 생성해둔 해시 테이블(해시 맵)을 탐색하면서 조인한다는 점이 다르다
    * 즉, NL조인처럼 건건이 Inner 집합을 버퍼캐시에서 탐색하지 않기 때문에 더 효율적이다
* 조인하는 양쪽 집합이 모두 대량 데이터일 때, 양쪽 모두 PGA 공간을 초과하더라도, 대량 데이터를 조인할 때는 NL조인보다 해시조인이 빠르다

<br>

### Hash 조인 특징 (vs Sort Merge 조인)
* 조인하는 양쪽 집합 중 한쪽이 다른 한쪽에 비해 현저히 작을 때 Hash 조인이 Sort Merge 조인보다 빠르다
