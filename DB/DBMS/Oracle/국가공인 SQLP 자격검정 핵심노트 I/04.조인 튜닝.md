...
### 종합실행계획
```sql
leading(b, a, c, d) use_hash(a) use_nl(c) use_hash(d) swap_join_inputs(d) index(a 가입상품_X1) index(c 가입부가상품_PK) index(d 상품_PK)
```
```sql
...
### 스칼라 서브쿼리 > 일반 조인문으로 변환
* 스칼라 서브쿼리
* 일반 조인문
* 결과집합 0건 
  * 스칼라 서브쿼리: 스칼라 값 null 리턴
  * 일반 조인문: 로우 0건 리턴
  * 따라서, 스칼라 서브쿼리 to 일반 조인문으로 변환할 때
    * Inner Join 사용 불가능 X
    * Outer Join 사용 필요 O
