...
* Local Index
  ...
  * 인덱스 칼럼 구성도에 따라서, Index Range Scan 연산의 범위가 달라질 수 있다
    * ex) 파티션키가 선두 칼럼이고 Index Access 조건이면, 해당 인덱스 파티션을 모두 읽어야 한다
    * ex) 파티션키가 후방 칼럼이고 Index Access 조건이면, 특정 범위만 읽어도 된다
