...
* 생략 가능한 인덱스 구성 조건
  * `=` 연산자로 사용한 조건절 칼럼 선정
    * `in` 연산자는 `=`이 아니다
    * 물론, IN-LIST ITERATOR 연산으로 빠져서 `=` 연산자로 변경되고, UNION ALL로 두 집합을 결합하는 쿼리로 변환 가능하다
      * 다만, 이럴 때, 결합된 두 집합의 정렬 순서를 인덱스로 제어하지 못하기 때문에, 결론적으로 IN-LIST ITERATOR 방식으로 풀려서는 안된다
  * order by절에 기술한 칼럼 추가
