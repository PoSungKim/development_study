...
* fetch call
  ...
  * 부분범위 처리 O
  * 네트워크 패킷 크기 증가 X
    * SDU (session data unit), TDU (transport data unit)은 서버와 클라이언트 간의 네트워크 IO관련인데, 이미 fetch call 횟수가 정해졌고, 그 하나 하나 각각의 fetch call 횟수 내의 네트워크 IO 설정이기에, fetch call 횟수 자체가 감소하지 않는다
