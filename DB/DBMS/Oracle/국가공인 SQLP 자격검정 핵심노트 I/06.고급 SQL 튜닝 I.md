...
### 특정 장비의 최종 상태코드, 변경일자, 변경순번
* 환경 
  * 테이블 (장비) --> 특정 장비구분코드를 가진 N개의 장비번호 존재 가능
    * (PK) 장비번호
    * 장비명
    * 장비구분코드
  * 테이블 (상태변경이력)
...
* 쿼리
  ...
  from     (
      select   상태변경이력.상태코드, 상태변경이력.변경일자, 상태변경이력.변경순번
      from     장비, 상태변경이력
      where    장비.장비구분코드 = 'A001'
      and      장비.장비구분코드 = 상태변경이력.장비구분코드
      order by 상태변경이력.변경일자 desc, 상태변경이력.변경순번 desc
  )
  where rownum <= 1;
  ```
