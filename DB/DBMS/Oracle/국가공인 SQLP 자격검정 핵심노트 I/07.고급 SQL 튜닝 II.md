...
* 쿼리
  ...
  ```
* 힌트
  * /*+ leading(A) use_hash(B) full(A) full(B) parallel(A 2) parallel(B 2) PQ_DISTRIBUTE(B, BROADCAST, NONE) */
* Broadcast 데이터 재분배를 통한 병렬 조인
...
* 쿼리
  ...
  ```
* 힌트
  * /*+ leading(A) use_hash(B) full(A) full(B) parallel(A 2) parallel(B 2) PQ_DISTRIBUTE(B, HASH, HASH) */
* 
  * 
