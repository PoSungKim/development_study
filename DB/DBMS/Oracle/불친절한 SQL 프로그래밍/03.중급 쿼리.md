  ...
  * PIVOT절은 FOR절의 IN절에 다중 열 기입 가능
    ...
    PIVOT  (SUM (sal) AS sal , COUNT (*) as cnt
            FOR (deptno, job) IN ((10, 'ANALYST') AS d10a, (10, 'CLERK') AS d10c,
                                  (20, 'ANALYST') AS d20a, (20, 'CLERK') AS . 20c))
    ORDER BY 1;
    ```
* 기존 방식
  * DECODE 방식
...
* 기본 문법
  ...
  * DEFAULT 세팅: EXCLUDE
  ...
  ```
  * UNPIVOT 절은 지정되지 않은 열을 기준으로 집계되기 때문에 SELECT절에 사용할 열을 지정해야 한다
  ```sql
  ...
  * 별칭 사용시 deptno 열의 유효값으로 들어간다
  ...
  OROER BY 1, 2;
  ```
  * INCLUDE NULLS: salary가 NULL인 행도 노출
  ```sql
  SELECT  job, deptno, sal
  FROM    t1
...
* 활용예제 
  * 동적 PIVOT
    * 동적 PIVOT은 쿼리 수행 시점에 동적으로 열이 변경되는 PIVOT
  * 전체 열 UNPIVOT
    * 테이블의 전체 열을 UNPIVOT하여 값 조회 및 집계 가능
    ```sql
    WITH w1 AS (
  ...
  * 행렬 전환 집계
    ...
    * 관계형 DBMS는 행 단위로 연산을 하기 때문에, 열을 행으로 전환해서 사칙연산으로 계산을 완료하고 다시 열로 전환할 수 있다
