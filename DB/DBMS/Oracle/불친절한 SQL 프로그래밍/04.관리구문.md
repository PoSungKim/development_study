...
* 기본 문법
  * CREATE VIEW 문
    ...
    -- ORA-01779: 키-보존된것이 아닌 테이블로 대응한 열을 수정할 수 없습니다
    -- Key-Preserved: "조인(Join)을 한 결과물에서도, 특정 테이블의 기본키(PK)가 여전히 유일(Unique)하게 유지되는가"
    -- t2.c1:t3.c1 간의 관계가 1:M이기 때문에, t2 테이블에 INSERT 불가능
    -- 첫 번째 INSERT는 괜찮지만, 추후에 (3, 3, 4)처럼 또 다시 넣으려고 하면, t2.c1에 이미 3이 있기 때문에 유일성이 보장되지 못함으로, View 형태로 INSERT를 아예 막아 버린다는 의미
    ...
    ```
  * WITH CHECK OPTION
    * 서브 쿼리의 WHERE 절에 해당하지 않은 데이터가 생성되는 것을 방지한다
    * 뷰를 통해서 실제 테이블을 수정을 했으나 막상 조회가 안되는 상황을 예방하기 위한 기능이라고 볼 수 있다
      ```sql
  ...
  * WITH READ ONLY
    * 읽기 전용으로 뷰 생성
      ...
      ```
  * 오브젝트 종속성
    * 오브젝트는 다른 오브젝트와 종속성을 가질 수 있다
      ```sql
      SELECT name, type, referenced_owner, referenced_name, referenced_type
      FROM   user_dependencies
      WHERE  name in ('V1', 'V2');
      -- V1, VIEW, SCOTT, T1, TABLE
      -- V2, VIEW, SCOTT, T2, TABLE
      -- V3, VIEW, SCOTT, T3, TABLE
      ```
  * ALTER VIEW 문
    ```sql
    ...
    SELECT * FROM v1;
    -- ORA-04063: view "SCOTT.V1" 에 에러가 있습니다
    -- v1 뷰가 t1 테이블의 c2 칼럼을 보기 때문에 에러가 발생한다
...
* 활용 예시
  * 데이터 보안성
    * v_emp 뷰에 대한 권한을 부여받은 사용자는 뷰의 열과 조건에 해당하는 데이터만 조회할 수 있으며， DML 작업도 해당 데이터에 대해서만 수행할 수 있다
      ...
      ```
  * 데이터 독립성
    * 데이터베이스 = 내부 스키마(Data File) + 개념 스키마(Table) + 외부 스키마(View) 
      ```sql
...
* 기본문법
  * CREATE SEQUENCE 문
    ...
    CREATE SEQUENCE [schema.]sequence
          ...
          |{SESSION | GLOBAL}];
    ```
      * INCREMENT BY integer: 시퀀스 사이의 간격 (기본값: 1)
      * START WITH integer: 시퀀스의 시작 번호 (기본값: 1)
      * MAXVALUE integer: 시퀀스가 생성할 수 있는 최대 값
      * NOMAXVALUE: 시퀀스가 오름차순이면 10^27, 내림차순이면 1 (기본 세팅)
      * MINVALUE integer: 시퀀스가 생성할 수 있는 최소 값
      * NOMINVALUE: 시퀀스가 오름차순이면 1, 내림차순이면 -10^26 (기본 세팅)
      * CYCLE: 최대 값이나 최소 값에 도달했을 때 순환해서 생성
      * NOCYCLE: 최대 값이나 최소 값에 도달했을 때 에러를 발생 (기본값)
      * CACHE integer | NOCACHE: 미리 할당해서 메모리에 저장하는 값의 개수 (기본값: 20)
    ...
    * 슈도 칼럼
    ...
    -- ORA-08002: 시퀀스 S1.CURRVAL은 이 세션에서는 정의 되어 있지 않습니다.
    ...
    FROM   user_sequences
    WHERE  sequence_name = '51';
    -- 1, 9999999999999999999999999999, 20, 21, 1
    ```
  * ALTER SEQUENCE 문
    ```sql
    ...
    SELECT s1.NEXTVAL AS c1 FROM DUAL;
    -- 100
    ALTER SEQUENCE 51 INCREMENT BY 1;
    SELECT s1.NEXTVAL AS c1 FROM DUAL;
    -- 101
...
* 시권스 유형
  * 전역 시퀀스
    * 시스템 레벨의 시퀀스로 전역 시퀀스는 시스템에 종속되며 여러 세션에서 공유할 수 있다
      ```sql
      DROP SEQUENCE s1;
      CREATE SEQUENCE s1;
      ```
      ...
      --S2: 2
      SELECT s1.NEXTVAL AS c1 FROM DUAL;
      --S1: 3 >> NEXTVAL은 세션 간에 공유 O
      SELECT s1.CURRVAL AS c1 FROM DUAL;
      --S2: 2 >> CURRVAL은 세션 간에 공유 X 
      ```
  * 세션 시퀀스
    * 세션 레벨의 시퀀스로 세션에 종속되며 세션이 종료되면 초기화된다
      ```sql
      DROP SEQUENCE s1;
...
* 신규 기능
  ...
  * IDENTITY 절
    ...
    * IDENTITY열은 테이블 당 1개만 생성 가능
      ...
      WHERE  table_name = 'T1';
      -- C2, ALWAYS, ISEQ$$_######, START WITH: 1, INCREMENT BY: 1, ...

      SELECT nullable, data_default, identity_column
      FROM   user_tab_columns
      WHERE  table_name  = 'T1'
        AND  column_name = 'C2';
      -- N, "SCOTT"."ISEQ$$_######".nextval, YES

      SELECT min_value, max_value, increment_by, cache_size, last_number
      FROM   user_sequences
      WHERE  sequence_name = 'ISEQ$$_######';
      -- 1, 9999999999999999999999999999, 20, 21
      ```
      ```sql
...
* 활용 예제
  * 시퀀스와 다중 테이블 INSERT 문
    ...
    -- CURRVAL 슈도 칼럼만 기술한 마지막 쿼리(c1 = 4) 외에는 모두 순차 값이 입력된 것을 확인 가능
    -- 헷갈리니, NEXTVAL만 사용하는 것이 바람직
    SELECT * FROM t1;
    -- 1, 1
    -- 1, 2
    -- 2, 3
    ...
    ```
    ```sql
    CREATE OR REPLACE FUNCT10N f1 RETURN NUMBER
    IS
    BEGIN
    RETURN s1.NEXTVAL;
    END f1;
...
### 시너님 (Synonym)
* 데이터베이스 오브젝트에 동의어를 부여할 수 있는 오브젝트다
* 기본 문법
  * CREATE SYNONYM
    ```sql
  ...
  * DROP SYNONYM
    ...
    ```
* 시너님 유형
  * 전용 시너님 (Private Synonym)
    * 특정 사용자에 종속된 시너님
  * 공용 시너님 (Public Synonym)
    * 모든 사용자가 사용할 수 있는 시너님
      ```sql
      -- SYS 사용자가 소유하고 있는 테이블로, DUAL은 공용 시너님이 있기 때문에, 스키마를 기술하지 않아도 사용할 수 있다
      ...
      SELECT * FROM dual;
      -- 선택된 레코드가 없습니다.
      -- SYS 소유자의 dual이 아니라, 현재 소유자의 dual테이블이다
      -- 소유자 오브젝트 > 전용 시너님 > 공용 시너님 순으로 해석한다
...
* 원격 데이터베이스의 오브젝트를 액세스할 수 있는 오브젝트
  * 주로 분산 데이터베이스 (Distributed Databse) 환경에서 사용
* 기본 문법
  * CREATE DATABASE LINK 문
    ...
    ```
      * 접속 사용자 방식: 현재 접속한 사용자와 동일한 사용자의 패스워드로 접속
    ...
    -- ORA-02063: line가 선행됨 (DL2 로 부터)

    SELECT db_link, username, host FROM user_db_links;
    -- DL1,      , ora12cr2
    -- DL2, SCOTT, ora12cr2
    ```
  * ALTER DATABASE LINK 문
    ```sql
 ...
 * 고급 주제
   * 세션 관리
     * DB 링크를 사용하면 원격 데이터베이스에 새로운 세션이 생성된다
       ```sql
       SELECT * FROM t1@dl1;
       -- 1
       ...
       COMMMIT;
       ROLLBACK;
       -- in_transaction이 YES면, 커밋 혹은 롤백으로 트랜잭션을 끊어줄 수 있다
       SELECT db_link, logged_on, open_cursors, in_transaction FROM v$dblink;
       -- DL1, YES, 0, NO
       ...
       -- ORA12CR2, HRJEONG-PC, 127.0.0.1, 2050-01-01 00:00:00.000000, 1
       ```
   * 오브젝트 관리
     * DB 링크를 사용하는 오브젝트에 대한 시너님을 생성하고 시너님을 통해 오브젝트를 액세스하는 편이 바람직하다
       ```sql
       CREATE OR REPLACE SYNONYM t1_dl1 FOR t1@d11;
       SELECT * FROM t1_dl1;
       -- 1
       ```
...
### COMMENT 문
* 테이블， 뷰， 열 등의 데이터베이스 오브젝트에 주석을 생성한다
  ```sql
...
### RENAME 문
* 테이블, 뷰, 시퀀스, 전용 시너님의 이름을 변경한다
  ```sql
  DROP VIEW emp_v;
  CREATE OR REPLACE VIEW v_emp AS SELECT * FROM empl
...
## DCL 문
#### Data Control Language, 데이터에 대한 권한을 부여하거나 취소할 수 있다

<br>

### 사용자(User)
* 데이터베이스에 로그인할 수 있는 계정
  * 시용자는 데이터베이스 오브젝트를 소유할 수 었다
  * 사용자가 소유하고 있는 오브젝트의 논리적인 집합이 스키마(schema)
* 기본 문법
  * CREATE USER
    ```sql
    ...
    | ACCOUNT {LOCK|UNLOCK}];
    ```
      * IDENTIFIED BY : 사용자의 패스워드 지정
      * DEFAULT TABLESPACE : 사용자의 기본 테이블스페이스 지정
      * TEMPORARY TABLESPACE : 사용자의 기본 임시 테이블스페이스 지정
      * QUOTA : 사용자가 할당할 수 있는 테이블스페이스의 한도 설정
...
* 기본 문법
  * GRANT 문
    * 시스템 권한
      ```sql
      GRANT {system_privilege | role | ALL_PRIVILEGES}
         [, {system_privilege | role | ALL_PRIVILEGES}]...
    ...
    * 오브젝트 권한
      ...
      FROM   dba_tab_privs
      WHERE  grantee = 'U2';
      -- U1, T1, U1, INSERT, NO, TABLE
      -- U1, T1, U1, SELECT, NO, TABLE
      ```
      ```sql
      SELECT owner, table_name, column_name, grantor, privilege, grantable
      FROM   dba_col_privs
      WHERE  grantee = 'U2';
      -- U1, T1, C2, U1, UPDATE, NO
    
      SELECT object_type_name, privilege_name
  ...
  * REVOKE문
    * 시스템 권한
      ```sql
      REVOKE {system_privilege | role | ALL_PRIVILEGES}
           [, {system_privilege | role | ALL_PRIVILEGES}]...
      FROM {user | role | PUBLIC} [, user | role | PUBLIC}]...;
      ```
      ```sql
      REVOKE CREATE TABLE, UNLIMTED TABLESPACE FROM u2;
      ```
    * 오브젝트 권한
      ```sql
      REVOKE {system_privilege | role | ALL_PRIVILEGES}
      ...
      ```
        * CASCADE CONSTRAINT: FK제약 조건과 관련하여 REFERENCES 권한을 취소
...
* 기본 문법
  * CREATE ROLE문
    ...
    SELECT   owner, table_name, grantor, privilege, grantable, type
      FROM   dba_sys_privs
      WHERE  grantee = 'R2';
    -- U1, T2, U1, INSERT, NO, TABLE
    -- U1, T2, U1, SELECT, NO, TABLE
    ```
  * ALTER ROLE문
    ```sql
  ...
  * DROP ROLE문
    ...
    ```
  * 권한 중복 부여
    * 권한은 사용자와 롤에 부여할 수 있기 때문에 중복되어 부여될 수 있다
      ```sql
      -- CREATE SESSION >> U2, R1
      -- R1 >> U2
      WITH w1 AS (
  ...
  * SET ROLE
    * 현재 세션에서 롤을 활성화하거나 비활성화한다
      ...
      FROM   dba_roles
      WHERE  role = 'R1';
      -- R1, YES --> 비밀번호가 세팅되어 있는 것을 확인할 수 있다
      ```
      ```sql
      -- USER: U2
      sqlplus u2/u2@ora12cr2
      ...
      -- U1 유저의 R1 롤이 활성화되서 CREATE TABLE권한 획득
      CREATE TABLE t3 (c1 NUMBER);
      ...
      ```
  * DEFAULT ROLE
    * 사용자의 기본 롤을 지정하거나 해제할 수 있다
      * 패스워드가 설정되지 않은 롤은 부여받은 사용자의 기본롤로 설정된다
      ```sql
      ...
      ```
  * 권한과 롤
    * 권한(시스템 권한, 오브젝트 권한, 칼럼 레벨 권한)은 사용자와 롤에 부여할 수 있다
    * 롤은 롤에 부여할 수 있다
    * 관련 뷰 테이블
      * _SYS_PRIVS
      * _TAB_PRIVS
...
* 문법
  ...
  ``` 
    * ADVISE: 원격 데이터베이스에 권고를 보내 분산 트랜잭션을 제어
...
* ALTER SESSION SET 절
  * 초기화 파라미터(initialization parameter)
    * 데이터베이스 기동 시 시용되는 파라미터
    ...
    ```
  * 뷰 목록
    * NLS_DATABASE_PARAMETERS: 데이터베이스의 영구적인 NLS 파라미터
    * NLS_INSTANCE_PARAMETERS: 인스턴스의 NLS 파라미터
    * NLS_SESSION_PARAMETERS: 사용자 세션의 NLS 파라미터 (VSNLS_PARAIETER 뷰와 동일)
  * 세션 레벨에서 수정할 수 있는 파라미터 (일부 파라미터는 다른 파라미터의 영향을 받는다)
    * NLS_LANGUAGE
      * 언어
      * NLS_LANG 환경 변수
    * NLS_TERRITORY
...
* NLS_TERRITORY 파라미터
  ...
  SELECT parameter, value
   ...
   WHERE parameter IN (
        'NLS_TERRITORY'
        , 'NLS_CURRENCY'
        , 'NLS_DUAL_ CURRENCY', 'NLS_ISO_CURRENCY'
  ...
  -- L: 지역 통화 기호(Local)
  -- U: 이중 통화 기호(Dual)
  -- C: ISO 국제 통화 코드(ISO Currency)

  SELECT DATE '2050-01-01'
       , TIMESTAMP '2050-01-01 00:00:00'
       , T1MESTAMP '2050-01-01 00:00:00 +09:00'
  FROM DUAL;
  -- 01-JAN-50, 01-JAN-50 12.00.00.000000000 AM, 01-JAN-50 12.00.00.000000000 AM +09:00
  ```
* NLS_DATE_FORMAT 파라미터
  * DATE 형식을 설정한다
    * 날짜 값의 반환 형식과 형식이 지정되지 않은 변환 함수의 기본 포뱃으로 사용된다
  ```sql
  ...
  -- scott
  -- 날짜형 칼럼을 다룰 때는 TO_DATE(문자열, 'YYYYMMDD')처럼 명시적으로 형변환을 해주는 것이 BEST PRACTICE
  SELECT ename FROM emp WHERE hiredate = TO_DATE ('19870419', 'YYYYMMDD');
  -- scott
  ```
* 세션 파라미터
  * 세션 레벨에서만 설정할 수 있는 파라미터
  * CONSTRAINT[S] 파라미터
    * DEFFERABLE 제약 조건의 상태를 설정한다
    ```sql
  ...
  * CURRENT_SCHEMA 파라미터
    ...
    ```sql
    SELECT * FROM t1;
    -- 1 >> scott.t1 테이블을 조회한다
    ALTER SESSION SET CURRENT SCHEMA = u1;
    SELECT * FROM t1;
    -- 2 >> u1.t1 테이블을 조회한다

    SELECT SYS_CONTEXT ('USERENV', 'SESSION_USER')   AS C1
         , SYS_CONTEXT ('USERENV', 'CURRENT_SCHEMA') AS C2 FROM DUAL;
    -- SCOTT, U1
    ```
    
  * ISOLATION_LEVEL 파라미터
    ```sql
  ...
  * TIME_ZONE 파라미터
    * 세션 시간대를 설정한다
    ...
    -- +09:00, 2050-01-01 09:00:00, 2050-01-01 09:00:00

    ALTER SESSION SET TIME_ZONE = '+08:00';

    SELECT SESSIONTIMEZONE AS c1, SYSDATE AS c2, CURRENT_DATE AS c2 FROM DUAL;
    -- +08:00, 2050-01-01 09:00:00, 2050-01-01 08:00:00
    ```
