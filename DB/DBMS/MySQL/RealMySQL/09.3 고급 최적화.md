...
* MRR 배치 키 액세스(mrr & batched_key_access) 
  * Driven Table에 엑세스할 때, 인덱스를 타지 못하면, Random I/O가 매번 테이블 풀 스캔을 할텐데, 그것은 너무나 큰 PayLoad이기 때문에, 메모리 속 `조인버퍼`에 넣어서 조인을 걸어준다
    * 드라이빙 테이블의 레코드를 기준으로 rowid 들을 정렬해서 Random I/O가 아닌 Sequence I/O로 드리븐 테이블의 레코드들과 조인될 수 있게 하는 것이 포인트
  * MRR (multi-range read)를 이용한 조인이 BKA (batch_key_access)조인이다
...
* 인덱스 컨디션 푸시다운(index_condition_pushdown)
  ...
  * index_condition_pushdown이 ON 설정일 때
    * 인덱스를 범위 제한 조건으ㅡ로 사용하지 못한다고 하더라도 인덱스에 포함된 칼럼의 조건이 있다면 모두 같이 모아서 스토리지 엔진으로 전달할 수 있게 핸드러 API가 개선됨 
