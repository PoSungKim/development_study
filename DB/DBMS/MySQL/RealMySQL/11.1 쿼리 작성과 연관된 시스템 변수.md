...
* SELECT
  ...
  * 잠금을 사용하는 SELECT
    * FOR SHARE (Shared Lock, 공유잠금) 
      * UPDATE/DELETE/ALTER 불가능 + SELECT FOR SHARE 가능 + SELECT 가능(InnoDB Non Locking Consistent Read)
    * FOR UPDATE (Exclusive Lock, 배타잠금)
      * UPDATE/DELETE/ALTER 불가능 + SELECT FOR SHARE 불가능 + SELECT 가능(InnoDB Non Locking Consistent Read)
    * 잠금 테이블 선택
