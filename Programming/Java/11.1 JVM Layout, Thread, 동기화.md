...
* 상황
  * OPEN인 티켓 1장에 고객 n명이 경쟁해서 고객 1명이 구매한 케이스
    ...
    * 다만, 최초에는 모든 티켓이 OPEN이라서 모든 참여자가 앞순서부터 OPEN인 티켓만 보게된다
    * 물론, 한 바퀴 다 돌고, 후진입한 고객에 한해서는 CLOSE인 티켓에 대한 불필요한 synchronized 연산이 발생하지 않는다
  * 따라서, OPEN인 티켓에 한해서만 동기화를 하되, 선점당한 OPEN 티켓이 CLOSE 상태면 continue, 즉 스킵하게 한 로직이다
    * 그럼에도, synchronous blocking하게 작동하기 때문에, 멀티쓰레드 환경은 갖춰졌으나, 병렬처리까지는 못하고 있는 상황이다
