...
### [dispatcher-servlet.xml - Spring MVC 기능]
```xml
...
```
* 조심해야 하는 부분은 dispatcher-servlet.xml 설정과 java annotation 설정을 함께 사용하면, xml 파일이 나중에 적용되서 java annotation은 전혀 적용되지 않는다 (모두 overwrite된다는 점)
