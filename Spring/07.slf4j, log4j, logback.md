...
|요소|설명|
|------|---|
|Logger|실제 로그 기능을 구현하는 객체|
|Appender|각 이벤트에 대한 로그 출력 위치를 결정하는 객체(콘솔, 파일, 메일, DB 등)|
...
|RollingFileAppender|FileAppender을 상속받아 패턴에 따라 각각의 다른 파일에 출력|
|SMTPAppender|메일로 발송|
|DBAppender|DB에 적재|
...
### [log4j의 로그 레벨 : FATAL > ERROR > WARN > INFO > DEBUG > TRANCE]
