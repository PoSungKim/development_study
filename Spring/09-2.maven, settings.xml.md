...
### [maven-jar-plugin, maven-assembly-plugin, maven-shade-plugin, spring-boot-maven-plugin]
```xml
<plugin>
  <artifactId>maven-jar-plugin</artifactId>
  <version>3.0.2</version>
...
```
* `mvn package` 명령어 실행시, `MANIFEST.MF` 파일에 `Main-Class: ` 값을 주입해줌으로써 Executable Jar 파일을 생성할 수 있게 해준다
