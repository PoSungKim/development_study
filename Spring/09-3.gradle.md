...
```
* `buildscript` : build.gradle, 즉 Application이 아닌 Task 실행 시 외부 라이브러리를 사용할 때 (보통, plugins) --> subprojects에서도 사용
...
* `subprojects`
  * `ext` : extra properties로 `${ext명}` 형태로 값을 가져와서 사용 가능
