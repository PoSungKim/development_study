...
#### 상위 클래스의 하나의 코드가 하위 클래스에서 여러 자료형으로 구현되어 실행되는 것 
...
## 8. Multi-thread 환경에서 동기화 진행, 동기화를 진행하는 시스템 함수 (semaphore?)
...
## 10. Java 10부터 local variable inference 가능 (다만 처음에 선언한 데이터 타입에서 다른 타입으로 중간에 변환 불가능)
