...
### [하위 Class가 상위 Class를 확장시킨다, Extend 시킨다 - 단일 상속만 가능]
```java
Class B extends A {
  ... 
}
```
* 구조
  * 상속하는 Class A : ```상위 클래스```, ```Parent Class```, Base Class, Super Class
  * 상속받는 Class B : ```하위 클래스```, ```Child Class```, Derived Class, Subclass
