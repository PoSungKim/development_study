...
### [<프로젝트명>Application.java | Base64Encoder와 UrlEncoder만 ```@Component``` 해준 상태]
```java
...
### [<프로젝트명>Application.java | Encoder도  ```@Component``` 해준 상태]
```java
@SpringBootApplication
...
## 5. SQL Mapper - Hibernate (dto, vo) vs MyBatis (DAO), oom vs erd
