...
* ```DTO (Data Transfer Object)``` : 말그대로 ```Transfer```를 위해서 Layer 사이의 통신을 위한 객체
  * 정보 교환을 위한 기능 --> 메소드의 ```return과 parameter``` 값으로 사용
* ```DAO (Data Access Object)```   : 말그대로 ```Access```를 위해서 DB와 통신하는 객체, ```Entity Class```라고도 부름
